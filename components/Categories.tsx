...
export default function Categories() {
  ...
  const renderCard = (it: Item, i: number, ariaHidden: boolean) => {
    ...
    return (
      <motion.div
        ...
        whileTap={{ scale: 0.97 }}
        whileHover={{ y: -6 }}
        className="shrink-0 pr-4 md:pr-5"
  ...
  return (
    <section className="bg-bone py-12 md:py-16 overflow-hidden">
      ...
      <div className="relative">
        <div className="absolute inset-y-0 left-0 w-16 md:w-32 z-10 bg-gradient-to-r from-bone to-transparent pointer-events-none" />
        <div className="absolute inset-y-0 right-0 w-16 md:w-32 z-10 bg-gradient-to-l from-bone to-transparent pointer-events-none" />

        <div
          onMouseEnter={() => setPaused(true)}
          onMouseLeave={() => setPaused(false)}
          className="flex whitespace-nowrap animate-marquee md:animate-marquee-slow"
          style={{ animationPlayState: paused ? "paused" : "running" }}
        >
          <div className="flex shrink-0">
            {items.map((it, i) => renderCard(it, i, false))}
          </div>
          <div className="flex shrink-0">
            {items.map((it, i) => renderCard(it, i + items.length, true))}
          </div>
        </div>
      </div>
    </section>
